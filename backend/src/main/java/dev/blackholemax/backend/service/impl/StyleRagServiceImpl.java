...
/**
 * 轻量 RAG 检索服务实现：基于 TF-IDF 余弦相似度匹配用户输入与风格提示词库。
 * 不依赖外部向量数据库或 embedding API，纯内存计算，零外部依赖。
 */
@Service
public class StyleRagServiceImpl implements StyleRagService {

    private static final Logger log = LoggerFactory.getLogger(StyleRagServiceImpl.class);
    ...
    public StyleRagServiceImpl(StylePromptRepository stylePromptRepository) {
